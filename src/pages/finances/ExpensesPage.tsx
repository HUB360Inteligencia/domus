...
export default function ExpensesPage() {
  ...
  const transactions = allTransactions.filter(tx => tx.transaction_type === 'expense');
  ...
  const handleOpenModal = (transaction?: any) => {
    if (transaction) {
      ...
      // Pré-configura para despesa quando criar novo
      setSelectedTransaction({ ...transaction, transaction_type: 'expense' });
    }
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (data: TransactionFormData) => {
    // Garante que é sempre uma despesa
  ...
  const resetFilters = () => {
    handleFilterChange({
      startDate: undefined,
      endDate: undefined,
      type: ['expense'], // Mantém filtro por despesas
      category: undefined,
  ...
  // Calcular total
  const expenseTotal = transactions.reduce((sum, tx) => sum + Number(tx.amount), 0);

  return (
    <div className="container py-6">
      <PageHeader title="Gestão de Despesas" description="Controle e análise de todas as suas despesas">
